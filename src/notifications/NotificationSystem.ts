...
const EMAIL_FUNCTION_URL = import.meta.env.VITE_CALENDAR_EMAIL_FUNCTION_URL as string | undefined;
...
export function formatEventReminderEmail(event: CalendarEvent): EventEmailPayload {
...
export async function sendEventEmailReminder(event: CalendarEvent, user: User): Promise<void> {
  if (!EMAIL_FUNCTION_URL) {
    throw new Error("Calendar email function URL is not configured (VITE_CALENDAR_EMAIL_FUNCTION_URL).");
  }
  if (!user.email) {
