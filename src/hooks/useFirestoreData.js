...
export const useUpdateProfile = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      qc.setQueryData(["profile", user?.uid], (old) => ({
        ...(old || {}),
...
export const useCreateSubject = () => {
  ...
  return useMutation({
    ...
    onSuccess: (created) => {
      ...
      qc.setQueryData(["subjects", user?.uid], (old) => [
        ...(old ?? []),
        {
          id: created.id,
...
export const useDeleteSubject = () => {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id) => await fs.deleteSubject(user.uid, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["subjects"] }),
  });
};

// ==================== SCHEDULES ====================
export const useSchedules = (dayOfWeek) => {
  const { user } = useAuth();
  return useQuery({
...
export const useDeleteSchedule = () => {
  ...
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id) => await fs.deleteSchedule(user.uid, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["schedules"] }),
  });
};
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return sessions.map((s) => {
        const subj = subjectById.get(s.subjectId);
        const startedAt =
          s.startTime?.toDate?.()?.toISOString?.() ?? new Date().toISOString();
        return {
          id: s.id,
          user_id: s.userId,
          subject_id: s.subjectId,
          ...
          subjects: subj
            ? { name: subj.subjectName, color: subj.color ?? "#6366F1" }
            : undefined,
        };
      });
    },
...
// ==================== DAILY PROGRESS ====================
export const useDailyProgress = (days = 30) => {
  const { user } = useAuth();
  return useQuery({
...
export const useStudyEvents = (year, month) => {
  const { user } = useAuth();
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: (_created, variables) => {
      ...
      qc.invalidateQueries({
        queryKey: ["study_events", user?.uid, d.getFullYear(), d.getMonth()],
      });
    },
  });
};
