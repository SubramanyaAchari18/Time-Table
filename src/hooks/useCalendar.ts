...
export const useCreateCalendarEvent = () => {
  const { user } = useAuth();
  return useMutation({
    mutationFn: async (payload: { title: string; description?: string; eventType: CalendarEventType; eventDate: string }) => {
...
export const useUpdateCalendarEvent = () => {
  return useMutation({
    mutationFn: async (payload: { id: string; title: string; description?: string; eventType: CalendarEventType }) => {
...
export const useUpcomingCalendarEvents = (daysAhead: number) => {
  const { user } = useAuth();
  const [events, setEvents] = useState<CalendarEvent[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
