      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          {completed
            ...
            : `You studied ${subjectName} for ${formatDuration(durationSeconds)}`}
        </p>
      </div>
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((s) => (
          <button key={s} className="p-1" onClick={() => setRating(s)}>
            <Star
              className={`h-7 w-7 transition-colors ${s <= rating ? "text-warning fill-warning" : "text-muted-foreground/30 hover:text-warning"}`}
            />
          </button>
        ))}
      </div>
