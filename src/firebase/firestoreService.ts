...
// -------------------- users --------------------
export async function createUserProfile(user: User, extra?: { name?: string }) {
  const userRef = doc(firestoreDb, "users", user.uid);
  const name = extra?.name ?? user.displayName ?? user.email?.split("@")[0] ?? "Student";
  await setDoc(
    userRef,
    {
      name,
...
// -------------------- subjects --------------------
export async function addSubject(userId: string, subject: { subjectName: string; studyDuration: number; color?: string; priority?: string }) {
  const payload = {
    userId,
...
export async function deleteSubject(_userId: string, subjectId: string) {
  await deleteDoc(doc(firestoreDb, "subjects", subjectId));
}

// -------------------- schedules (needed for timetable UI) --------------------
export async function addSchedule(userId: string, schedule: { subjectId: string; dayOfWeek: number; startTime: string; endTime: string }) {
  const payload = {
    userId,
...
export async function getSchedules(userId: string, dayOfWeek?: number) {
  const constraints: any[] = [where("userId", "==", userId), orderBy("startTime", "asc")];
  if (dayOfWeek !== undefined) constraints.unshift(where("dayOfWeek", "==", dayOfWeek));
  const q = query(collection(firestoreDb, "schedules"), ...constraints);
  const snaps = await getDocs(q);
  return snaps.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
}

export async function deleteSchedule(_userId: string, scheduleId: string) {
...
export async function getStudySessions(userId: string, limit = 50) {
  const q = query(collection(firestoreDb, "studySessions"), where("userId", "==", userId), orderBy("startTime", "desc"));
  const snaps = await getDocs(q);
  return snaps.docs.slice(0, limit).map((d) => ({ id: d.id, ...(d.data() as any) }));
}

export async function getDailyProgress(userId: string, days: number) {
  ...
  const byDate = new Map<string, { total_study_seconds: number; sessions_completed: number; gems_earned: number }>();
  for (const docSnap of snaps.docs) {
    const row: any = docSnap.data();
    const dt = row.startTime?.toDate?.() ?? new Date();
    const dateStr = toLocalDateKey(dt);
    const prev = byDate.get(dateStr) ?? { total_study_seconds: 0, sessions_completed: 0, gems_earned: 0 };
...
export async function updateStreak(userId: string, studyDate: Date = new Date()) {
  ...
  if (!snap.exists()) {
    await setDoc(refDoc, { currentStreak: 1, longestStreak: 1, lastStudyDate: todayStr }, { merge: true });
    return;
  }

  const data: any = snap.data();
  const lastStr = data.lastStudyDate as string | undefined;
  ...
  await setDoc(refDoc, { currentStreak: newCurrent, longestStreak: newLongest, lastStudyDate: todayStr }, { merge: true });
}

export async function getStreak(userId: string): Promise<number> {
  const snap = await getDoc(doc(firestoreDb, "streaks", userId));
  return snap.exists() ? Number((snap.data() as any).currentStreak ?? 0) : 0;
}

// -------------------- notes folders/files (Firebase only) --------------------
export async function createFolder(userId: string, folder: { name: string }) {
...
export async function getFolders(userId: string) {
  const q = query(collection(firestoreDb, "folders"), where("userId", "==", userId), orderBy("createdAt", "asc"));
  const snaps = await getDocs(q);
  return snaps.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
}
...
export async function getNotesUsage(userId: string): Promise<{ bytesUsed: number; limitBytes: number }> {
  const usageRef = doc(firestoreDb, "notesUsage", userId);
  const snap = await getDoc(usageRef);
  if (snap.exists()) {
    const data: any = snap.data();
    return { bytesUsed: Number(data.bytesUsed ?? 0), limitBytes: NOTES_STORAGE_LIMIT_BYTES };
  }

  // Fallback: compute from existing notes, then persist for future fast reads.
  const qFiles = query(collection(firestoreDb, "notes"), where("userId", "==", userId));
  const filesSnap = await getDocs(qFiles);
  const bytesUsed = filesSnap.docs.reduce((sum, d) => sum + Number((d.data() as any).fileSizeBytes ?? 0), 0);
  await setDoc(usageRef, { bytesUsed, updatedAt: serverTimestamp() }, { merge: true });
  return { bytesUsed, limitBytes: NOTES_STORAGE_LIMIT_BYTES };
}

export async function uploadNotesFile(params: { userId: string; folderId?: string | null; file: File }) {
  ...
  await runTransaction(firestoreDb, async (tx) => {
    ...
    const used = usageSnap.exists() ? Number((usageSnap.data() as any).bytesUsed ?? 0) : 0;
    ...
    tx.set(usageRef, { bytesUsed: used + params.file.size, updatedAt: serverTimestamp() }, { merge: true });
    tx.set(fileDocRef, {
      userId: params.userId,
  ...
  try {
    ...
    await updateDoc(fileDocRef, { downloadUrl, status: "ready", updatedAt: serverTimestamp() });
    return {
      id: fileDocRef.id,
      userId: params.userId,
      folderId: params.folderId ?? null,
    ...
    try {
      await runTransaction(firestoreDb, async (tx) => {
        const usageSnap = await tx.get(usageRef);
        const used = usageSnap.exists() ? Number((usageSnap.data() as any).bytesUsed ?? 0) : 0;
        tx.set(usageRef, { bytesUsed: Math.max(0, used - params.file.size), updatedAt: serverTimestamp() }, { merge: true });
...
export async function getNotesFiles(userId: string, folderId?: string) {
  ...
  return items.sort((a: any, b: any) => {
    const aTs = (a.createdAt as Timestamp | undefined)?.toMillis?.() ?? 0;
    const bTs = (b.createdAt as Timestamp | undefined)?.toMillis?.() ?? 0;
    return bTs - aTs;
  });
}

export async function deleteNotesFile(_userId: string, fileId: string) {
  ...
  if (usageRef) {
    await runTransaction(firestoreDb, async (tx) => {
      const usageSnap = await tx.get(usageRef);
      const used = usageSnap.exists() ? Number((usageSnap.data() as any).bytesUsed ?? 0) : 0;
      tx.set(usageRef, { bytesUsed: Math.max(0, used - fileSizeBytes), updatedAt: serverTimestamp() }, { merge: true });
      tx.delete(fileRef);
    });
  } else {
...
const DEFAULT_ACHIEVEMENTS = [
  { id: "1", name: "First Steps", description: "Complete your first study session", icon: "🌟", rarity: "common", requirement_type: "sessions", requirement_value: 1, gems_reward: 5 },
  { id: "2", name: "Dedicated", description: "Complete 5 study sessions", icon: "📚", rarity: "common", requirement_type: "sessions", requirement_value: 5, gems_reward: 15 },
  { id: "3", name: "Week Warrior", description: "Study 7 days in a row", icon: "🔥", rarity: "rare", requirement_type: "streak", requirement_value: 7, gems_reward: 50 },
  { id: "4", name: "Gem Collector", description: "Earn 100 gems", icon: "💎", rarity: "rare", requirement_type: "gems", requirement_value: 100, gems_reward: 25 },
];
